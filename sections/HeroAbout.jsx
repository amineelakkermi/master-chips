...
// Réduit le travail de ScrollTrigger pendant le scroll rapide
...
export default function HeroAbout() {
  ...
  // Cache des valeurs de transition — recalculées seulement au refresh
  const transitionCache = useRef({ x: 0, y: 0, scale: 1 });
  const carouselDistance = useRef(0);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* ── Mesures coûteuses : faites UNE fois par refresh, pas par frame ── */
      const measureTransition = () => {
        ...
        if (!wrapper || !target || !fixed) return;
        ...
        const chipW   = chipImg?.offsetWidth || chipImg?.naturalWidth || 1;

        transitionCache.current = {
          x: imgCenterX - W / 2,
          y: imgCenterY - H / 2,
          scale: target.offsetWidth / chipW,
        };
      };
      ...
      const measureCarousel = () => {
        ...
        if (!lastCard) { carouselDistance.current = 0; return; }
        // mesure depuis x=0
        const prevX = gsap.getProperty(el, "x");
        gsap.set(el, { x: 0 });
        const lastRect = lastCard.getBoundingClientRect();
        const contRect = container.getBoundingClientRect();
        carouselDistance.current =
          lastRect.right - contRect.right + window.innerWidth * 0.15;
        gsap.set(el, { x: prevX });
      };

      // Recalcule toutes les mesures à chaque refresh (resize, load, etc.)
      ...
      gsap.to(leftBadgeRef.current,  { y: -14, duration: 2.2, repeat: -1, yoyo: true, ease: "sine.inOut", force3D: true });
      gsap.to(rightBadgeRef.current, { y:  14, duration: 3.2, repeat: -1, yoyo: true, ease: "sine.inOut", force3D: true });
      ...
      // Promotion GPU des éléments scrubés
      gsap.set([productRef.current, productCardRef.current], { willChange: "transform" });
      ...
      }).to(productRef.current, { x: "28vw", scale: 0.78, rotation: 10, ease: "none", force3D: true });

      /* ─── ABOUT → PRODUCTS Phase 1 : drift vers la carte (valeurs cachées) ─── */
      gsap.timeline({
        scrollTrigger: {
          trigger: aboutRef.current,
          start: "top top",
        ...
        x: () => transitionCache.current.x,
        y: () => transitionCache.current.y,
        scale: () => transitionCache.current.scale,
        rotation: 0,
        ease: "none",
        force3D: true,
      });

      /* ─── Phase 2 : crossfade serré à l'arrivée ─── */
      gsap.timeline({
        scrollTrigger: {
          trigger: aboutRef.current,
          start: "85% top",
        ...
        .to(firstCardImageRef.current, { opacity: 1, ease: "none" }, 0)
        .to(productRef.current,        { opacity: 0, ease: "none" }, 0);
      ...
      /* ─── STATS COUNT-UP ─── */
      statsRef.current?.querySelectorAll("[data-count]").forEach((node) => {
        const end = parseFloat(node.dataset.count);
        const isFloat = String(end).includes(".");
        gsap.fromTo(node, { innerText: 0 }, {
          innerText: end,
          duration: 2,
          ease: "power2.out",
          ...
          onUpdate() {
            node.innerText = isFloat
              ? parseFloat(node.innerText).toFixed(1)
              : Math.round(parseFloat(node.innerText));
          },
        });
      });
    ...
    // Refresh quand les images Next.js sont chargées (mesures correctes)
    ...
    <div ref={containerRef} className="relative bg-[#0B1E2D]">
      ...
      <section ref={heroRef} className="relative min-h-screen overflow-hidden">
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse 70% 60% at 50% 40%, rgba(232,184,75,0.08) 0%, transparent 70%)" }}
        />
        <div className="relative z-10 mx-auto max-w-7xl px-6 md:px-10">
          <div className="relative flex min-h-screen flex-col items-center justify-center">
            <div ref={eyebrowRef} className="mb-6 flex items-center gap-3">
              <span className="h-px w-10 bg-[#E8B84B]/60" />
              <span className="font-mono text-xs tracking-[0.25em] uppercase text-[#E8B84B]/80">Est. 1998 · Algeria</span>
              <span className="h-px w-10 bg-[#E8B84B]/60" />
            </div>
            <div className="relative text-center">
              <h1 ref={headlineRef} className="font-paprika leading-[0.92] text-[clamp(3.5rem,12vw,9rem)] text-white">The Taste</h1>
              <h2 ref={sublineRef} className="font-paprika leading-[0.92] text-[clamp(3.5rem,12vw,9rem)]">
                <span className="text-[#E8B84B]">Algeria</span>{" "}<span className="text-white">Loves</span>
              </h2>
            </div>
            <p ref={heroSubtitleRef} className="mt-8 max-w-sm text-center text-base leading-7 text-white/50 md:text-lg">
              Crafted with passion, enjoyed across Algeria. One crunch at a time.
            </p>
            <div ref={leftBadgeRef} className="absolute left-0 top-[30%] hidden -rotate-12 md:flex">
              <span className="rounded-full border border-[#E8B84B]/30 bg-[#E8B84B]/10 px-8 py-3 text-sm font-medium tracking-wide text-[#E8B84B] backdrop-blur-sm">Delicious</span>
            </div>
            <div ref={rightBadgeRef} className="absolute bottom-[12%] right-0 hidden rotate-12 md:flex">
              <span className="rounded-full border border-white/20 bg-white/5 px-8 py-3 text-sm font-medium tracking-wide text-white/70 backdrop-blur-sm">Since 1998</span>
            </div>
      ...
      {/* FIXED FLOATING CHIP — shadow sur un wrapper, transform sur le parent */}
      <div
        ref={productRef}
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[998] pointer-events-none"
      >
        <div style={{ filter: "drop-shadow(0 30px 45px rgba(232,184,75,0.22))" }}>
          <Image
            src={masterChips}
            alt="Master Chips"
            priority
      ...
      {/* ABOUT */}
      <section ref={aboutRef} className="relative z-30 min-h-screen bg-[#091729] flex flex-col items-center justify-center px-6 py-24 md:px-10">
        <div aria-hidden className="pointer-events-none absolute right-6 top-16 text-[clamp(6rem,20vw,14rem)] font-black leading-none text-white/[0.03] select-none">1998</div>
        <div className="relative max-w-5xl w-full">
          <div className="mb-10 flex items-center gap-3">
            <span className="h-px w-8 bg-[#E8B84B]/50" />
            <span className="font-mono text-xs tracking-[0.22em] uppercase text-[#E8B84B]/70">Our Story</span>
          </div>
          <p ref={textRef} className="font-paprika text-[clamp(1.35rem,3vw,2.2rem)] leading-[1.55] text-white/85">{ABOUT_TEXT}</p>
      ...
      >
        <div aria-hidden className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 50% at 50% 100%, rgba(232,184,75,0.05) 0%, transparent 60%)" }} />
        <div className="relative w-full max-w-7xl px-6 md:px-10">
          <div className="mb-14 flex flex-col items-center text-center">
            <div className="mb-4 flex items-center gap-3">
              <span className="h-px w-8 bg-[#E8B84B]/50" />
              <span className="font-mono text-xs tracking-[0.22em] uppercase text-[#E8B84B]/70">The Range</span>
              <span className="h-px w-8 bg-[#E8B84B]/50" />
            </div>
            <h2 className="font-paprika text-[clamp(2.5rem,7vw,5rem)] leading-tight text-white">Our <span className="text-[#E8B84B]">Products</span></h2>
          </div>
          <div className="overflow-hidden">
            <div ref={productCardRef} className="flex gap-8">
              {PRODUCTS.map(({ image, name, tagline, desc, badge, price, accent }, i) => (
                ...
                >
                  {/* backdrop-blur retiré → fond solide semi-opaque (gros gain de perf) */}
                  <div
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-center p-8 md:p-12 flex-1">
                      <p className="mb-2 text-xs tracking-[0.2em] uppercase" style={{ color: accent }}>Master Chips</p>
                      <h3 className="font-paprika text-[clamp(1.6rem,3.5vw,2.4rem)] leading-tight text-white">{name.replace("Master Chips ", "")}</h3>
                      ...
                      <div className="mt-8 flex items-center justify-between">
                        <span className="font-paprika text-2xl font-bold" style={{ color: accent }}>{price}</span>
                        <button
                          ...
                          onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = accent; e.currentTarget.style.color = "#0B1E2D"; }}
          ...
          <p className="mt-8 text-center text-xs tracking-[0.15em] uppercase text-white/25">Scroll to explore</p>
        </div>
      </section>
    </div>
  );
